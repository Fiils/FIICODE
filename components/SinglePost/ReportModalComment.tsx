...
const ReportModal = ({ originalPostId, commentId, setReport, setReportModal }: { setReport: Dispatch<SetStateAction<boolean>>, setReportModal: Dispatch<SetStateAction<boolean>>, originalPostId: string, commentId: string }) => {
    ...
    const ReportRequest = async (e: any) => {
        if(auth.user.active) {
            ...
            const result = await axios.patch(`${server}/api/comment/report/${originalPostId}/${commentId}`, { reason }, { withCredentials: true })
                                        .then(res => res.data)
                                        .catch(err => {
                                            console.log(err);
                                            setLoading(false)
