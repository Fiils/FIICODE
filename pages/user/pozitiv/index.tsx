...
const Positive: NextPage<InitialFetchProps> = ({ posts }) => {
    ...
    useEffect(() => {
        const getMorePosts = async () => {
            const result: Post = await axios.get(`http://localhost:9999/api/user/upvotes?page=${addPosts}`, { withCredentials: true })
                            .then(res => res.data)
                            .catch(err => {
                                setError(true)
                                console.log(true)
...
export const getServerSideProps: GetServerSideProps = async (ctx: any) => {
    ...
    const posts = await axios.get(`http://localhost:9999/api/user/upvotes`, { withCredentials: true, headers: { Cookie: ctx.req.headers.cookie || 'a' } })
                        .then(res => res.data)
                        .catch(err => {
                            console.log(err.response) 
                            redirect = true
